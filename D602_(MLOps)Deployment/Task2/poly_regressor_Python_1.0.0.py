...
# configure logger
logname = "Output/polynomial_regression.txt"
...
def create_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # delete delays > 1h
    df2['DEPARTURE_DELAY'] = df2['DEPARTURE_DELAY'].astype(float)
...
f = open("Output/airport_encodings.json","w")
...
with mlflow.start_run(run_name = 'Test Data Run'):
    # Log input parameters
    mlflow.log_param("alpha", parameters[0] / 10)
    mlflow.log_param("polynomial_order", parameters[1])

    # Log model performance metrics
    mlflow.log_metric("Test Data Mean Squared Error", score)
    mlflow.log_metric("Test Data Average Delay", np.sqrt(score))

    # Log the performance plot artifact
    mlflow.log_artifact("Output/model_performance_test.jpg")

    # Log additional artifacts
    mlflow.log_artifact("Output/polynomial_regression.txt")
